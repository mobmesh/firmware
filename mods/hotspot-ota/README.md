...
| `stop ota`                                      | Shuts down the access point that upstream's `start ota` raises. Refused while an upload is writing. The access point also shuts itself down 20 minutes after `start ota`, or 10 minutes after an upload stops making progress, so an abandoned session no longer stays open until the node reboots. Only one `start ota` works per boot either way: upstream never releases port 80, so a second one is refused rather than raising an access point with nothing behind it. |
