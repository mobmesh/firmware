...
**Fragility note:** this depends on overriding Arduino-ESP32 core's weak `verifyRollbackLater()` symbol (see `RollbackGuard.cpp`) with an exact name and signature match. If a future Arduino-ESP32 core bump ever renames or changes that hook, the override silently stops linking — no build error, no runtime error. The failure mode is benign (every image just auto-confirms immediately again, i.e. stock upstream's original behavior before this patch existed), but it's a silent loss of the safety net, not a loud one. The build workflow greps the installed core for that symbol after every build and fails loudly if it's gone (see `.github/workflows/build-release.yml`) — that catches the symbol vanishing or being renamed, but not a core change that keeps the symbol but stops calling it, so a periodic hardware sanity check (flash a build that would fail `radio_init()` and confirm it actually rolls back) after any core version bump is still worthwhile.
