...
<sub>**Fragility note:** this depends on overriding Arduino-ESP32 core's weak `verifyRollbackLater()` symbol (see `RollbackGuard.cpp`). The build workflow counts occurrences of that symbol in the installed core after every build and fails loudly if there are fewer than 2 (the weak prototype plus at least one call site) — catching both the symbol vanishing entirely and a core that keeps the declaration but stops calling it. It can't catch a core that renames the *caller's* logic in some other way that still happens to reference the same symbol name incidentally (e.g. in a comment), so a hardware sanity check (flash a build that would fail `radio_init()`, confirm it actually rolls back) is still worth doing after a core version bump.</sub>
