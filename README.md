...
**Fragility note:** this depends on overriding Arduino-ESP32 core's weak `verifyRollbackLater()` symbol (see `RollbackGuard.cpp`). The build workflow greps the installed core for that symbol after every build and fails loudly if it's gone or renamed. It can't catch a core that keeps the symbol but stops calling it, though, so a hardware sanity check (flash a build that would fail `radio_init()`, confirm it actually rolls back) is still worth doing after a core version bump.
