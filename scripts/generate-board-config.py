...
def read_section_values(ini_path: Path, section: str, key: str) -> list[str]:
    """Return a multi-line ini value as a list, e.g. [esp32_ota]'s lib_deps entries."""
    lines = ini_path.read_text().splitlines()
    try:
        start = lines.index(f"[{section}]")
    except ValueError:
        sys.exit(f"error: [{section}] not found in {ini_path}")
    values = []
    collecting = False
    for line in lines[start + 1:]:
        ...
        if line.split("=")[0].strip() == key:
            ...
            if head:
                values.append(head)
    if not values:
        sys.exit(f"error: [{section}] in {ini_path} has no '{key}' entries")
    return values


def render_ota_web_page(html_path: Path) -> str:
    """Emit the header the mod compiles its OTA page from.

    The page ships as HTML so review sees markup rather than a byte array; the gzip blob and the
    C array are produced here. mtime=0 keeps the output reproducible across builds.
    """
    blob = gzip.compress(html_path.read_bytes(), mtime=0)
    rows = [", ".join(f"0x{b:02x}" for b in blob[i:i + 16]) for i in range(0, len(blob), 16)]
    body = ",\n  ".join(rows)
    return (
        f"// Generated by generate-board-config.py inject-env from {html_path.name}. Do not edit.\n"
        "#pragma once\n\n#include <Arduino.h>\n\n"
        f"static const uint32_t MOBMESH_OTA_PAGE_LEN = {len(blob)};\n"
        "static const uint8_t MOBMESH_OTA_PAGE[] PROGMEM = {\n  " + body + "\n};\n"
    )
...
def cmd_inject_env(args):
    ...
    # Driven by the selected mods, not by board configuration: a target that omits the mod keeps
    ...
    if ota_page_mod:
        ...
        all_build_flags_lines = all_build_flags_lines + ["-D DISABLE_WIFI_OTA"]
    ...
    if ota_page_mod:
        # The mod serves the page and identity itself, so upstream's OTA web library -- ~53 KB
        # of embedded page -- is dropped, keeping the rest of upstream's own dependency list.
        definition = load_mod_definition(ota_page_mod)
        page = REPO_ROOT / "mods" / ota_page_mod / definition.ota_web_page
        if not page.exists():
            sys.exit(f"error: {ota_page_mod}'s ota_web_page not found at {page}")
        header = ini_path.parent.parent.parent / "src" / "helpers" / "esp32" / "OtaWebPage.h"
        header.parent.mkdir(parents=True, exist_ok=True)
        header.write_text(render_ota_web_page(page))

        ld_idx = find_key_line(section_lines, "lib_deps")
        if ld_idx is None:
            sys.exit(f"error: 'lib_deps =' not found in [env:{env}] section of {ini_path}")
        ld_end = find_value_block_end(section_lines, ld_idx)
        upstream_ota_deps = read_section_values(
            ini_path.parent.parent.parent / "platformio.ini", "esp32_ota", "lib_deps"
        )
        kept = [d for d in upstream_ota_deps
                if not (d.startswith("file://") and d.rstrip("/").endswith("AsyncElegantOTA"))]
        if len(kept) == len(upstream_ota_deps):
            sys.exit(
                "error: [esp32_ota] lib_deps carries no file:// AsyncElegantOTA entry to drop "
                f"-- upstream layout changed, cannot honour {ota_page_mod}'s ota_web_page"
            )
        swapped = False
        for i in range(ld_idx, ld_end + 1):
            if "${esp32_ota.lib_deps}" in section_lines[i]:
                section_lines[i] = "".join(f"{indent}{d}\n" for d in kept)
                swapped = True
                break
        if not swapped:
            sys.exit(
                f"error: [env:{env}] in {ini_path} has no ${{esp32_ota.lib_deps}} entry to rewrite "
                f"for {ota_page_mod}'s ota_web_page"
            )
